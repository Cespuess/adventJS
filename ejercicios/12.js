function checkIsValidCopy(original, copy) {
  ...
  */
  function isInRegExp(characterToSearch) {
    let pattern = `^[${characterToSearch}#+:. ]$`;
  ...
  for (let i = 0; i < original.length; i++) {
    ...
    else if (original[i] === ' ' && copy[i] !== original[i]) return false;
    else if (!isNaN(original[i]) && copy[i] !== original[i]) return false;
    else if (/^[A-Z]$/.test(original[i])) {
      const character = original[i].toLowerCase();
      const regex = isInRegExp(character);
      if (!regex.test(copy[i])) return false;
    } else if (degradation.includes(original[i])) {
      const index = degradation.indexOf(original[i]);
      const searchArray = degradation.slice(index + 1);
      if (!searchArray.includes(copy[i])) return false;
    } else {
      const regex = isInRegExp(original[i]);
      if (!regex.test(copy[i])) return false;
    }
  }
